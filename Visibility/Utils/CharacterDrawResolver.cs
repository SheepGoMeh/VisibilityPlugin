...
namespace Visibility.Utils
{
	...
	internal class CharacterDrawResolver
	{
		...
		public unsafe void Init(DalamudPluginInterface pluginInterface, VisibilityConfiguration pluginConfig)
		{
			...
			hookCharacterEnableDraw = new Hook<CharacterEnableDrawPrototype>(_address.CharacterEnableDrawAddress,
				new CharacterEnableDrawPrototype(CharacterEnableDrawDetour), this);
			hookCharacterDisableDraw = new Hook<CharacterDisableDrawPrototype>(_address.CharacterDisableDrawAddress,
				new CharacterDisableDrawPrototype(CharacterDisableDrawDetour), this);
			hookCompanionEnableDraw = new Hook<CompanionEnableDrawPrototype>(_address.CompanionEnableDrawAddress,
				new CompanionEnableDrawPrototype(CompanionEnableDrawDetour), this);
			hookCharacterDtor = new Hook<CharacterDtorPrototype>(_address.CharacterDtorAddress,
				new CharacterDtorPrototype(CharacterDtorDetour), this);
		...
		private unsafe void CharacterEnableDrawDetour(Character* thisPtr)
		{
			...
			if (!_config.Enabled || !localPlayerAddress.HasValue || thisPtr == (Character*) LocalPlayer) return;
			switch (thisPtr->GameObject.ObjectKind)
			{
				case (byte) ObjectKind.Player:
					...
					if (!_config.HidePlayer ||
					    ...
					    (_config.ShowFriendPlayer &&
					     _players[ContainerType.Friend].Contains(thisPtr->GameObject.ObjectID)) ||
					    (_config.ShowCompanyPlayer &&
					     _players[ContainerType.Company].Contains(thisPtr->GameObject.ObjectID)) ||
					    (_config.ShowPartyPlayer &&
					     _players[ContainerType.Party].Contains(thisPtr->GameObject.ObjectID)))
					{
						break;
					}
					else
				...
				case (byte) ObjectKind.BattleNpc when thisPtr->GameObject.SubKind == (byte) BattleNpcSubKind.Pet &&
					...
					if ((_config.ShowFriendPet && _players[ContainerType.Friend].Contains(thisPtr->GameObject.OwnerID))
					    || (_config.ShowCompanyPet &&
					        _players[ContainerType.Company].Contains(thisPtr->GameObject.OwnerID))
					    || (_config.ShowPartyPet &&
				...
				case (byte) ObjectKind.BattleNpc when thisPtr->GameObject.SubKind == 3:
					...
					if ((_config.ShowFriendChocobo &&
					     _players[ContainerType.Friend].Contains(thisPtr->GameObject.OwnerID))
					    || (_config.ShowCompanyChocobo &&
					        _players[ContainerType.Company].Contains(thisPtr->GameObject.OwnerID))
					    || (_config.ShowPartyChocobo &&
					        _players[ContainerType.Party].Contains(thisPtr->GameObject.OwnerID)))
					{
						break;
					}
		...
		private unsafe void CompanionEnableDrawDetour(Companion* thisPtr)
		{
			    ...
			    && thisPtr->Character.CompanionOwnerID != LocalPlayer->Character.GameObject.ObjectID)
			{
				...
				if ((_config.ShowFriendMinion &&
				     _players[ContainerType.Friend].Contains(thisPtr->Character.CompanionOwnerID))
				    || (_config.ShowCompanyMinion &&
				        _players[ContainerType.Company].Contains(thisPtr->Character.CompanionOwnerID))
				    || (_config.ShowPartyMinion &&
				        _players[ContainerType.Party].Contains(thisPtr->Character.CompanionOwnerID)))
				{
				}
				else
				{
